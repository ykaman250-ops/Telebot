...
async def connect(bot: Bot, update):
    ...
    try:
        ...
        await update.reply_text(f"Make sure I am added as an Admin in the channel <code>{target}</code> first!\n\nError: {e}")
        return
    ...
    try:
        ...
        async for msgs in bot.get_chat_history(channel_id):
            if not (msgs.video or msgs.audio or msgs.document):
                continue
                
            try:
                if msgs.video:
                    file_id = msgs.video.file_id
                    file_name = msgs.video.file_name[0:-4] if msgs.video.file_name else "Video"
                    ...
                    file_name = msgs.audio.file_name[0:-4] if msgs.audio.file_name else "Audio"
                    file_caption = msgs.caption if msgs.caption else ""
                    ...
                    file_name = msgs.document.file_name[0:-4] if msgs.document.file_name else "Document"
                    file_caption = msgs.caption if msgs.caption else ""
...
async def disconnect(bot: Bot, update):
    ...
    try:
        ...
        await update.reply_text(f"I Couldnt Fetch Details Of `{target}` Make Sure I Am An Admin There And Try Again....!!")
        return
...
async def new_files(bot: Bot, update):
    ...
    try:
        if update.video: 
            ...
            file_name = update.video.file_name[0:-4] if update.video.file_name else "Video"
            ...
            file_name = update.audio.file_name[0:-4] if update.audio.file_name else "Audio"
            file_caption  = update.caption if update.caption else ""
            ...
            file_name = update.document.file_name[0:-4] if update.document.file_name else "Document"
            file_caption  = update.caption if update.caption else ""
